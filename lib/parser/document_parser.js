...
function getValidations(field) {
  ...
  for (let i = 0; i < field.validations.length; i++) {
    ...
    if (validation.constant) {
      validation.value = getConstantValue(validation.value);
    }
    let value = validation.value;
    if (validation.key === Validations.PATTERN) {
      value = formatPatternValidation(value);
    }
    if (validation.key === Validations.ACCEPTABLE_VALUES) {
      value = getAcceptableValues(validation.value);
    }
...
/**
 * Maps the list of acceptable value entries to acceptable values
 * converts any constants to their constant value
 * @param value - list of allowable values from the JDL
 */
function getAcceptableValues(value) {
  return value.map(
    acceptableValueEntry =>
      acceptableValueEntry.constant
        ? getConstantValue(acceptableValueEntry.acceptableValue)
        : acceptableValueEntry.acceptableValue
  );
}

/**
 * Gets a constant value from the configuration
