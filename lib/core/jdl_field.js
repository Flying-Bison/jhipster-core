...
class JDLField {
  constructor(args) {
     ...
     */
    Object.values(merged.validations).forEach(element => checkValidation(element, this.type));
  ...
  static checkValidity(field) {
    ...
    if (field.validations) {
      for (let i = 0; i < field.validations.length; i++) {
        const validationErrors = JDLValidation.checkValidity(field.validations[i], field.type);
...
function checkValidation(validation, fieldType) {
  const errors = JDLValidation.checkValidity(validation, fieldType);
  if (errors.length !== 0) {
