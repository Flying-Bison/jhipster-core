...
module.exports = class JDLParser extends Parser {
  ...
  parse() {
    ...
    this.pattern();
    this.acceptableValuesValidation();
    this.acceptableValuesValidationEntry();
  ...
  validation() {
    this.RULE('validation', () => {
      this.OR([
        ...
        { ALT: () => this.SUBRULE(this.acceptableValuesValidation) }
      ]);
    });
  }
  ...
  acceptableValuesValidation() {
    this.RULE('acceptableValuesValidation', () => {
      // Rule allows defining a list of acceptable values for a field ex: acceptableValues([0.01,0.02,0.05])
      this.CONSUME(LexerTokens.ACCEPTABLE_VALUES);
      this.CONSUME(LexerTokens.LPAREN);
      this.CONSUME(LexerTokens.LSQUARE);
      this.MANY(() => {
        this.SUBRULE(this.acceptableValuesValidationEntry);
        this.OPTION(() => {
          this.CONSUME(LexerTokens.COMMA);
        });
      });
  ...
  acceptableValuesValidationEntry() {
    this.RULE('acceptableValuesValidationEntry', () => {
      this.OR([
        { ALT: () => this.CONSUME(LexerTokens.FLOAT) },
