...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
  ...
  validation(context) {
    ...
    if (context.acceptableValuesValidation) {
      return this.visit(context.acceptableValuesValidation);
    }
  ...
  /**
   * Builds the acceptableValuesValidation from the context
   * Given acceptableValues([1,2,3]) in jdl the method should return:
   * {key:'acceptableValues', value:[{{acceptableValue:1,constant:false}},{{acceptableValue:2,constant:false}},{{acceptableValue:3,constant:false}}]}
   * @param context - current context
   * @returns {{key: string, value: Array}}
   */
  acceptableValuesValidation(context) {
    return {
      key: 'acceptableValues',
      value: context.acceptableValuesValidationEntry
        ? _.map(context.acceptableValuesValidationEntry, element => this.visit(element))
        : []
    };
  }

  /**
   * Extracts the individual values form the acceptableValues validation
   * Given acceptableValues([1,2,3]) in jdl the first call to this would return {acceptableValue:1,constant:false}
   * @param context - the current context defining one entry of the acceptable values list
   * @returns {{constant: boolean}}
   */
  acceptableValuesValidationEntry(context) {
    const isIdentifier = !!context.NAME;
    const acceptableValueAst = {
      constant: isIdentifier
    };

    if (context.NAME) {
      acceptableValueAst.acceptableValue = context.NAME[0].image;
    } else if (context.INTEGER) {
      acceptableValueAst.acceptableValue = parseInt(context.INTEGER[0].image, 10);
    } else if (context.FLOAT) {
      acceptableValueAst.acceptableValue = parseFloat(context.FLOAT[0].image);
    } else if (context.STRING) {
      const stringImage = context.STRING[0].image;
      acceptableValueAst.acceptableValue = stringImage.substring(1, stringImage.length - 1);
    }

    return acceptableValueAst;
  }
