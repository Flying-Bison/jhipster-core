...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
   ...
   * {key:'acceptableValues', value:[{{allowableValue:1,constant:false}},{{allowableValue:2,constant:false}},{{allowableValue:3,constant:false}}]}
   ...
   * Given acceptableValues([1,2,3]) in jdl the first call to this would return {allowableValue:1,constant:false}
   * @param context - the current context defining one entry of the allowable values list
   * @returns {{constant: boolean}}
   */
  acceptableValuesValidationEntry(context) {
    const isIdentifier = !!context.NAME;
    const allowableValueAst = {
      constant: isIdentifier
    };

    if (context.NAME) {
      allowableValueAst.allowableValue = context.NAME[0].image;
    } else if (context.INTEGER) {
      allowableValueAst.allowableValue = parseInt(context.INTEGER[0].image, 10);
    } else if (context.FLOAT) {
      allowableValueAst.allowableValue = parseFloat(context.FLOAT[0].image);
    } else if (context.STRING) {
      const stringImage = context.STRING[0].image;
      allowableValueAst.allowableValue = stringImage.substring(1, stringImage.length - 1);
    }

    return allowableValueAst;
  }
